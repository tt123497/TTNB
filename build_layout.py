#!/usr/bin/env python3
"""Build layout data with better sector-stock matching"""
import json, re
...
def match_sector(event_sector, event_name):
    # 1. Exact map
    if event_sector in sector_map:
        return sector_map[event_sector]
    # 2. Check aliases
    for alias, target in sector_map.items():
        if alias in event_sector or event_sector in alias:
            return target
    # 3. Fuzzy word overlap with D.groups sectors
    ...
    for sec_name in sector_stocks:
        ...
        if words & sec_words:
            return sec_name
    # 4. Check event name for clues
    if '芯片' in event_name or '半导体' in event_name or '存储' in event_name or 'HBM' in event_name:
        return 'HBM/存储芯片'
    if '固态' in event_name or '电池' in event_name:
        return '固态电池'
    if '机器人' in event_name or 'Optim' in event_name:
        return '人形机器人'
    if '卫星' in event_name or '航天' in event_name or 'SpaceX' in event_name or '火箭' in event_name:
        return '商业航天'
    if 'AI' in event_sector or 'AI' in event_name:
        return 'AI芯片'
    # 5. "全部赛道" -> top picks across all sectors
    if '全部赛道' in event_sector or '全部' in event_sector:
        all_picks = set()
        for sec_name, stocks in sector_stocks.items():
            for s in stocks[:2]:  # top 2 per sector
                all_picks.add(s)
        return None  # Special: return all top picks
    return None
...
for e in events:
    ...
    if '全部赛道' in e['s']:
        # Collect top picks from all sectors
        all_stocks = set()
        for sec_name, stocks in sector_stocks.items():
            for s in stocks[:2]:
                all_stocks.add(s)
        stocks = sorted(all_stocks)[:12]
    elif matched_sec:
...
# Count
empty = sum(1 for e in layout if not e['stocks'])
print('Layout: %d events, %d empty' % (len(layout), empty))
for ev in layout:
    if not ev['stocks']:
        d_str = ev['d']
        s_str = ev['s']
        e_str = ev['e'][:40]
        print('  EMPTY: %s | %s | %s' % (d_str, s_str, e_str))
