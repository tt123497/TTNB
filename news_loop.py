#!/usr/bin/env python3
"""每分钟拉新闻+推送, 无窗口运行"""
import os, sys, time, json, subprocess, re, uuid
...
SECTOR_KW = ['六氟化钨','WF6','AI芯片','GPU','HBM','CPO','硅光','光模块','PCB','MLCC','电子树脂','PPE','铜箔','HVLP','存储','液冷','服务器','数据中心','AIDC','半导体','光刻胶','先进封装','CoWoS','靶材','机器人','商业航天','SpaceX','卫星','固态电池','低空经济','eVTOL','电网','特高压','火电','风电','光伏','储能','锂矿','锂电池','新能源车','稀土','小金属','核能','量子','6G','连接器','AI眼镜','碳纤维','钨','钼','钠电池','玻璃基板','TGV','培育钻石','DrMOS','算电协同','Token工厂','医药','CRO','医疗器械','白酒','食品','银行','券商','保险','房地产','煤炭','黄金','铜铝','贵金属','钢铁','化工','超导','空间计算','交换机']
...
def fetch_json(url, hdrs=None, tm=10):
    try:
        h = {'User-Agent': UA, 'Referer': 'https://finance.sina.com.cn/'}
        if hdrs: h.update(hdrs)
        with urlopen(Request(url, headers=h), timeout=tm) as r:
            return json.loads(r.read().decode('utf-8'))
    except: return None

def now_cst(): return datetime.now(timezone.utc) + timedelta(hours=8)

def sina_news():
    cst, sn, mn = now_cst(), [], []
    for ch, nm in [('2512','股票'),('2516','A股'),('2509','7x24'),('1689','产业')]:
        d = fetch_json(f'https://feed.mix.sina.com.cn/api/roll/get?pageid=153&lid={ch}&k=&num=60&page=1&r={time.time()}')
        ...
        for it in d['result'].get('data',[]):
            ...
            if (cst-ts).total_seconds()/3600 > 2: continue
...
def em_ann():
    ...
    sn, mn = [], []
    for tp in ['A','SFA','SHA']:
        d = fetch_json(f'https://np-anotice-stock.eastmoney.com/api/security/ann?page_size=40&page_index=1&ann_type={tp}&sr=-1&client_source=web')
        ...
        for it in d.get('data',{}).get('list',[]):
            ...
            cds = it.get('codes',[]); sc = cds[0].get('stock_code','') if cds else ''
...
def wscn():
    ...
    for ch, nm in [('global-channel','全球'),('china-channel','中国')]:
        d = fetch_json(f'https://api-one.wallstcn.com/apiv1/content/lives?channel={ch}&client=pc&limit=40&first=1')
...
def dedup(ns):
    seen=set(); rs=[]
    for n in ns: k=n['t'][:50]; rs.append(n) if k not in seen else None; seen.add(k)
    rs.sort(key=lambda n: n.get('time',''), reverse=True)
    return rs

def git(cmd):
    try: return subprocess.run(cmd, shell=True, cwd=DIR, capture_output=True, timeout=15, creationflags=0x08000000).returncode
    except: return 1
...
while True:
    try:
        cst = now_cst()
        s1,m1 = sina_news(); s2,m2 = em_ann(); w = wscn(); g = em_7x24()
        ns, nm = dedup(s1+s2), dedup(m1+m2+w)

        data = {}
        if os.path.exists(DATA):
            try: data = json.load(open(DATA,'r',encoding='utf-8'))
            except: pass
        ...
        data['globalNews'] = g
        json.dump(data, open(DATA+'.tmp','w',encoding='utf-8'), ensure_ascii=False, indent=2)
        os.replace(DATA+'.tmp', DATA)

        git('git add data.json')
        git('git commit -m news')
